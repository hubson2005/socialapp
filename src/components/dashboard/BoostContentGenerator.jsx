...
import {
  ...
  Loader2, Check, Hash, FileText, Palette,
...
// ─── Generate post text via claude-proxy Edge Function ────────────────────────
async function generatePostText({ profile, network, boostType }) {
  const networkLabel = NETWORKS[network]?.label || network;
  const prompt =
    `Tu es un expert en marketing digital pour l'Afrique francophone.\n\n` +
    ...
    `- Maximum ${(NETWORKS[network]?.maxChars || 500) > 500 ? 280 : 150} caractères pour le texte principal\n` +
...
function generateSponsoredImage({ profile, boostType, canvasRef }) {
  return new Promise((resolve) => {
    ...
    grad.addColorStop(0, '#0f0a1e'); grad.addColorStop(0.5, '#1a0a3e'); grad.addColorStop(1, '#0a1628');
    ctx.fillStyle = grad; ctx.fillRect(0, 0, W, H);

    ctx.globalAlpha = 0.12;
    const c1 = ctx.createRadialGradient(200, 200, 0, 200, 200, 300);
    c1.addColorStop(0, '#6366f1'); c1.addColorStop(1, 'transparent');
    ctx.fillStyle = c1; ctx.beginPath(); ctx.arc(200, 200, 300, 0, Math.PI * 2); ctx.fill();
    const c2 = ctx.createRadialGradient(880, 880, 0, 880, 880, 280);
    c2.addColorStop(0, '#f59e0b'); c2.addColorStop(1, 'transparent');
    ctx.fillStyle = c2; ctx.beginPath(); ctx.arc(880, 880, 280, 0, Math.PI * 2); ctx.fill();
    ctx.globalAlpha = 1;

    const bg = ctx.createLinearGradient(750, 40, 1040, 40);
    ...
    ctx.textAlign = 'center'; ctx.fillText('⭐ SPONSORISÉ', 895, 73);
    ...
    const drawContent = () => {
      ctx.fillStyle = 'white'; ctx.font = 'bold 68px system-ui, sans-serif';
      ...
      const lines = wrapText(ctx, profile.bio || 'Professionnel sur SocialApp', W / 2, 620, 800);
    ...
    if (profile.avatar_url) {
      ...
      img.onload = () => {
        ...
        ctx.clip(); ctx.drawImage(img, ax - ar, ay - ar, ar * 2, ar * 2); ctx.restore();
        drawContent();
      };
      img.onerror = () => { drawInitials(ctx, ax, ay, ar, profile.display_name); drawContent(); };
      img.src = profile.avatar_url;
    } else {
      drawInitials(ctx, ax, ay, ar, profile.display_name);
      drawContent();
    }
  });
}
...
export default function BoostContentGenerator({ profile, boost, onContentReady }) {
  ...
  const handleGenerateText = useCallback(async (network) => {
    ...
    try {
      const result = await generatePostText({
        profile, network, boostType: boost?.boost_type || 'standard',
      });
  ...
  const handleGenerateImage = useCallback(async () => {
    ...
    try {
      const url = await generateSponsoredImage({ profile, boostType: boost?.boost_type || 'standard', canvasRef });
      setImageDataUrl(url);
  ...
  const isGenerating = Object.values(generating).some(Boolean) || generatingImage;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px' }}>
        ...
        <button onClick={handleGenerateAll} disabled={isGenerating} style={{
          display: 'flex', alignItems: 'center', gap: '6px', padding: '10px 18px',
          background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', border: 'none',
          borderRadius: '12px', color: 'white', fontSize: '12px', fontWeight: 700,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
            {boostNetworks.map(network => {
              const cfg = NETWORKS[network]; if (!cfg) return null;
              const Icon = cfg.icon; const isActive = activeNetwork === network;
              return (
                <button key={network} onClick={() => setActiveNetwork(network)} style={{
                  ...
                  color: isActive ? 'white' : 'rgba(255,255,255,0.5)', fontSize: '12px', fontWeight: 600, cursor: 'pointer',
                }}>
                  <Icon size={13} color={isActive ? cfg.color : 'rgba(255,255,255,0.4)'} />
                  {cfg.label}
                  {!!contents[network] && (
                    <div style={{ position: 'absolute', top: '-4px', right: '-4px', width: '10px', height: '10px', borderRadius: '50%', background: '#22c55e', border: '2px solid #0a0817' }} />
                  )}
                </button>
              );
            })}
          ...
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                ...
                <button onClick={() => handleGenerateText(activeNetwork)} disabled={generating[activeNetwork]} style={{
                  display: 'flex', alignItems: 'center', gap: '5px', padding: '6px 12px',
                  background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', border: 'none',
                  borderRadius: '8px', color: 'white', fontSize: '11px', fontWeight: 600, cursor: 'pointer',
                }}>
                  {generating[activeNetwork] ? <Loader2 size={11} className="animate-spin" /> : <RefreshCw size={11} />}
            ...
            <div style={{ padding: '16px' }}>
              {!currentContent ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {currentContent.hook && (
                    <div style={{ background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: '10px', padding: '10px 12px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '10px', padding: '12px', color: 'rgba(255,255,255,0.85)', fontSize: '13px', lineHeight: '1.6', whiteSpace: 'pre-wrap' }}>
                      {currentContent.text}
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                        {currentContent.hashtags.map((tag, i) => (
                          <span key={i} style={{ background: 'rgba(99,102,241,0.12)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: '6px', padding: '3px 8px', color: '#a78bfa', fontSize: '11px', fontWeight: 600 }}>
                            #{tag.replace('#', '')}
                          </span>
                        ))}
                      </div>
      ...
      {activeTab === 'image' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: '11px', fontWeight: 600 }}>VISUEL 1080×1080 — FORMAT STORY/POST</span>
              <div style={{ display: 'flex', gap: '6px' }}>
                {imageDataUrl && (
                  <button onClick={handleDownloadImage} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '6px 12px', background: 'rgba(34,197,94,0.12)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: '8px', color: '#22c55e', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}>
                    <Download size={11} /> Télécharger
                  </button>
                )}
                <button onClick={handleGenerateImage} disabled={generatingImage} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '6px 12px', background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', border: 'none', borderRadius: '8px', color: 'white', fontSize: '11px', fontWeight: 600, cursor: 'pointer' }}>
            ...
            <div style={{ padding: '16px', display: 'flex', justifyContent: 'center' }}>
              {!imageDataUrl && !generatingImage ? (
                <div style={{ width: '100%', maxWidth: '400px', aspectRatio: '1', background: 'rgba(255,255,255,0.03)', border: '2px dashed rgba(255,255,255,0.1)', borderRadius: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
                  <ImageIcon size={32} color="rgba(255,255,255,0.15)" />
                  <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '13px', margin: 0 }}>Cliquez sur "Générer" pour créer le visuel sponsorisé</p>
                </div>
              ) : generatingImage ? (
                ...
                <img src={imageDataUrl} alt="Visuel sponsorisé" style={{ width: '100%', maxWidth: '400px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)' }} />
              )}
              <canvas ref={canvasRef} style={{ display: 'none' }} />
            </div>
          </div>
          <div style={{ background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.15)', borderRadius: '12px', padding: '12px 14px', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
            <Sparkles size={14} color="#a78bfa" style={{ flexShrink: 0, marginTop: '1px' }} />
            <div>
              <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '12px', fontWeight: 600, margin: '0 0 3px' }}>Format optimisé pour les réseaux sociaux</p>
              <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '11px', margin: 0 }}>1080×1080px · Photo de profil + nom + bio + lien + badge Sponsorisé</p>
            </div>
          </div>
        </div>
      )}
