  ...
  return (
    <div className="min-h-screen bg-[#111215] text-white p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        <StatCard icon={<QrCode size={20} />}    title="Scans QR"        value={stats.scans}     />
        <StatCard icon={<Users size={20} />}     title="RSVP Confirmés"  value={stats.confirmed} />
        <StatCard icon={<Eye size={20} />}       title="Vues Profil"     value={stats.views}     />
        <StatCard icon={<BarChart3 size={20} />} title="Interactions"    value={stats.clicks}    />
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-[#1a1c21] border border-white/10 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-zinc-400 mb-2 block">Nom événement</label>
            ...
            <div>
              <label className="text-sm text-zinc-400 mb-2 block">Date événement</label>
              <input type="text" value={eventData.date}
                onChange={(e) => setEventData({ ...eventData, date: e.target.value })}
                className="w-full bg-[#22252c] border border-white/10 rounded-xl px-4 py-3 outline-none" />
            </div>
          </div>
          <div className="mt-4">
            <label className="text-sm text-zinc-400 mb-2 block">Localisation</label>
          ...
          <div className="mt-4">
            <label className="text-sm text-zinc-400 mb-2 block">Numéro WhatsApp RSVP</label>
            <input type="text" value={eventData.whatsapp}
              onChange={(e) => setEventData({ ...eventData, whatsapp: e.target.value })}
              className="w-full bg-[#22252c] border border-white/10 rounded-xl px-4 py-3 outline-none" />
          </div>
        </div>
        ...
        <div className="bg-[#1a1c21] border border-white/10 rounded-2xl overflow-hidden">
          <div className="relative h-56 overflow-hidden">
            <img src={gallery[0]} alt="event" className="w-full h-full object-cover" />
          ...
          <div className="p-5 space-y-4">
            ...
            <p className="text-sm text-zinc-400 leading-relaxed">{eventData.description}</p>
      ...
      <div className="mt-8 bg-[#1a1c21] border border-white/10 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-zinc-400 mt-1">Ajoutez plusieurs images pour votre profil public.</p>
          </div>
      ...
      <div className="mt-8 grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-[#1a1c21] border border-white/10 rounded-2xl p-6">
          <h2 className="text-xl font-bold mb-6 flex items-center gap-2">💌 RSVP WhatsApp</h2>
          <div className="space-y-4">
            ...
            <input type="tel" placeholder="Téléphone" value={form.phone}
              ...
              className="w-full bg-[#22252c] border border-white/10 rounded-xl px-4 py-3 outline-none" />
            <input type="number" placeholder="Nombre de personnes" value={form.guests}
        ...
        <div className="bg-[#1a1c21] border border-white/10 rounded-2xl p-6">
          <h2 className="text-xl font-bold mb-6">🎟️ Invités récents</h2>
          <div className="space-y-4">
            {[
              { name: "Awa Traoré",    guests: 2, status: "Confirmé"   },
              { name: "Kevin Kouassi", guests: 4, status: "En attente" },
              { name: "Sarah Yao",     guests: 1, status: "Confirmé"   },
            ].map((guest, index) => (
              <div key={index}
                className="flex items-center justify-between bg-[#22252c] border border-white/10 rounded-xl px-4 py-3">
                <div>
                  <h3 className="font-semibold">{guest.name}</h3>
                  <p className="text-sm text-zinc-400">👥 {guest.guests} personne(s)</p>
                </div>
