...
// ─── MetaLoginButton ──────────────────────────────────────────────────────────
function MetaLoginButton({ onSuccess }) {
  const [loading, setLoading] = useState(false);

  const handleLogin = () => {
    setLoading(true);
    if (!window.FB) {
      toast.error('SDK Facebook non chargé. Vérifiez votre App ID.');
      setLoading(false);
      return;
    }
  ...
  const fetchPagesAndInstagram = async (userToken) => {
    try {
      // 1. Récupère les pages Facebook de l'utilisateur
      ...
      const page = pagesData.data[0]; // Prend la première page
      const pageToken = page.access_token;
      const pageId = page.id;
      const pageName = page.name;

      // 2. Cherche le compte Instagram lié à cette page
  ...
  return (
    ...
    >
      {loading ? <Loader2 size={16} className="animate-spin" /> : <FaFacebook size={16} />}
      {loading ? 'Connexion en cours…' : 'Connecter Facebook & Instagram'}
    </button>
  );
}
          ...
          <div key={log.id} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <p style={{ color: 'rgba(255,255,255,0.3)', fontSize: '10px', margin: 0 }}>
                {log.published_at ? new Date(log.published_at).toLocaleString('fr-FR') : new Date(log.created_at).toLocaleString('fr-FR')}
              </p>
            </div>
            {isOk
              ? <div style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#22c55e' }}>
                  <CheckCircle size={12} />
                  <span style={{ fontSize: '10px', fontWeight: 600 }}>Publié</span>
                </div>
              : <div style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#ef4444' }}>
                  <XCircle size={12} />
                  <span style={{ fontSize: '10px', fontWeight: 600 }}>Échec</span>
                </div>
            }
          </div>
        );
...
export default function MetaIntegrationPanel({ profile, isAdmin = false }) {
  ...
  // Charge l'intégration existante
  useEffect(() => {
  ...
  const handleMetaConnected = async (metaData) => {
    try {
      ...
      // Désactive les anciennes intégrations
      await supabase.from('meta_integrations')
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const networks = Object.keys(result.results);
      const published = networks.filter(n => result.results[n]?.success !== false);
      const failed = networks.filter(n => result.results[n]?.success === false);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '680px' }}>
      ...
      {/* Status connexion */}
      <div style={{
        background: integration ? 'rgba(34,197,94,0.08)' : 'rgba(255,255,255,0.04)',
        border: '1px solid ' + (integration ? 'rgba(34,197,94,0.25)' : 'rgba(255,255,255,0.1)'),
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ color: 'rgba(255,255,255,0.6)', fontSize: '12px' }}>
                Connectez votre Page Facebook pour activer la publication automatique
              </span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
              {[
                [FaFacebook, '#1877f2', 'Facebook', 'Posts sur votre page'],
                [FaInstagram, '#e1306c', 'Instagram', 'Posts sponsorisés'],
                [FaWhatsapp, '#25d366', 'WhatsApp', 'Notifications'],
            ...
            <MetaLoginButton onSuccess={handleMetaConnected} />
          </div>
        )}
      </div>
      ...
      <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', padding: '20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        {isAdmin && (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: '12px' }}>
            ...
            <button onClick={() => setTestMode(v => !v)} style={{ width: '40px', height: '22px', borderRadius: '100px', background: testMode ? '#6366f1' : 'rgba(255,255,255,0.1)', border: 'none', cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}>
        ...
        <button
          ...
          disabled={publishing || (!selectedBoost && activeBoosts.length > 0) || (activeBoosts.length === 0)}
          style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px',
            padding: '13px',
            background: activeBoosts.length === 0 ? 'rgba(255,255,255,0.06)' : testMode ? 'linear-gradient(135deg,#6366f1,#8b5cf6)' : 'linear-gradient(135deg,#f59e0b,#ef4444)',
            border: 'none', borderRadius: '12px', color: activeBoosts.length === 0 ? 'rgba(255,255,255,0.3)' : 'white',
            fontSize: '13px', fontWeight: 700, cursor: activeBoosts.length === 0 ? 'not-allowed' : 'pointer',
          ...
          {showLogs && (
            <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} style={{ overflow: 'hidden' }}>
              <div style={{ padding: '0 16px 16px' }}>
                <PublicationLog boostId={selectedBoost} />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div style={{ background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.15)', borderRadius: '16px', padding: '16px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {[
            ['VITE_META_APP_ID', 'Votre App ID Meta dans .env', !!META_APP_ID && META_APP_ID !== 'YOUR_META_APP_ID'],
            ['Page Facebook', 'Avec un compte Instagram Business lié', !!integration?.facebook_page_id],
            ['Token valide', 'Reconnectez si expiré (90 jours)', !!integration?.facebook_access_token],
          ].map(([key, desc, ok]) => (
        ...
        <a href="https://developers.facebook.com/apps" target="_blank" rel="noopener noreferrer"
          style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', marginTop: '10px', color: '#a78bfa', fontSize: '11px', fontWeight: 600, textDecoration: 'none' }}>
