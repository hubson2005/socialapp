...
import {
  ...
  Loader2, AlertCircle, Crown, Inbox,
} from 'lucide-react';
...
const db = {
  list: async (profileId) => {
    const { data, error } = await supabase.from('forms').select('*').eq('profile_id', profileId).order('created_at', { ascending: false });
...
export default function FormsPanel({ profileId, maxForms = 1, onUpgrade }) {
  ...
  const handleNew = () => {
    if (atLimit) { toast.error(`Limite atteinte — ${maxForms} formulaire(s) max pour votre offre`); return; }
    setSelectedForm(null);
    setFormData(emptyForm(profileId));
    setTab('builder');
  };
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', minWidth: 0 }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        ...
                        <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '10.5px', margin: '2px 0 0' }}>{(form.fields || []).length} champ(s)</p>
                      </div>
                    </div>
                ...
                {selectedForm && (
                  <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '12px' }}>
                    ...
                    {selectedForm.status !== 'actif' && (
                      <p style={{ color: '#fbbf24', fontSize: '10.5px', margin: '0 0 8px' }}>⚠️ Passez le statut à "actif" pour rendre ce lien accessible publiquement.</p>
                    )}
