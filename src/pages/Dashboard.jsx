...
export default function Dashboard() {
  ...
  const handleSwitchProfile = useCallback(p => {
    if (hasChanges && !window.confirm('Modifications non sauvegardées. Continuer ?')) return;
  ...
  const handleDeleteProfile = useCallback(p => {
    if (!window.confirm('Supprimer "' + p.display_name + '" ?')) return;
  ...
  const handleSave = () => {
    ...
    const data = {
      ...
      event_tickets_enabled: localProfile.event_tickets_enabled || false,
    };
  ...
  const handleRemoveLink = useCallback(index => {
    ...
    updateLocal({ links });
    setLinksPage(p => Math.min(p, Math.max(0, Math.ceil(links.length / LINKS_PER_PAGE) - 1)));
  }, [localProfile, updateLocal]);

  const handleSignOut = async () => {
    if (hasChanges && !window.confirm('Modifications non sauvegardées. Se déconnecter ?')) return;
    await signOut();
  };

  const Toggle = ({ value, onChange }) => (
    <button onClick={() => onChange(!value)} style={{ width: '44px', height: '24px', borderRadius: '100px', background: value ? 'linear-gradient(135deg,#ff6b35,#f7c948)' : 'rgba(255,255,255,0.1)', border: 'none', cursor: 'pointer', position: 'relative', transition: 'background 0.3s', flexShrink: 0 }}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="bg-white/10 rounded-2xl border border-white/10 px-4 py-3 flex items-center justify-between gap-3">
              ...
              <Toggle value={localProfile.is_verified || false} onChange={v => updateLocal({ is_verified: v })} />
            </div>
            ...
            <div className="bg-white/10 rounded-2xl border border-white/10 px-4 py-3 space-y-3">
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/40 text-xs">Compte à rebours + vente de billets</p>
                  </div>
                </div>
                <Toggle value={localProfile.is_event || false} onChange={v => updateLocal({ is_event: v })} />
              </div>
              ...
              {localProfile.is_event && (
                <div className="space-y-2 pt-1 border-t border-white/10">
                  ...
                  {/* Vente de billets toggle */}
                  <div className="flex items-center justify-between px-1 py-1">
                    <div>
                      <span className="text-white/70 text-sm">🎫 Activer la vente de billets</span>
                      <p className="text-white/40 text-xs">Les visiteurs pourront acheter leurs billets directement</p>
                    </div>
                    <Toggle value={localProfile.event_tickets_enabled || false} onChange={v => updateLocal({ event_tickets_enabled: v })} />
                  </div>

                  {/* TicketManager */}
                  {localProfile.event_tickets_enabled && localProfile.id && (
                    <TicketManager profileId={localProfile.id} />
                  )}
                </div>
              )}
            </div>
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {pagedLinks.map((link, i) => {
                    ...
                    return <PlatformCard key={link.id || link.platform + '-' + absoluteIndex} link={link} index={absoluteIndex} onUpdate={updated => handleUpdateLink(absoluteIndex, updated)} onRemove={() => handleRemoveLink(absoluteIndex)} />;
                  })}
                </div>
                {totalLinkPages > 1 && (
