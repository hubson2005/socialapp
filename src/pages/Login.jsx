...
export default function Login() {
  ...
  const handleSocialAuth = async (provider) => {
    if (!supabase) return;
    const providerKey = provider === 'Google' ? 'google' : provider === 'Apple' ? 'apple' : 'facebook';
    try {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#1a1825', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', position: 'relative', overflow: 'hidden', fontFamily: "'Sora', 'Segoe UI', sans-serif" }}>
      ...
      <div className="scale-in" style={{ position:'relative', zIndex:1, width:'100%', maxWidth:'420px', background:'rgba(255,255,255,0.04)', backdropFilter:'blur(24px)', border:'1px solid rgba(255,255,255,0.09)', borderRadius:'28px', padding:'36px 32px', boxShadow:'0 32px 80px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.08)' }}>
        ...
        {/* ── Bandeau plan — MASQUÉ sur mobile, visible sur desktop ── */}
        {mode !== 'success' && (
          <div style={{ display:'none' }} className="plan-banner-desktop">
            {/* Le bandeau plan est masqué sur mobile */}
          </div>
        )}
            ...
            <div style={{ marginBottom:'20px' }}>
              <p style={{ color:'rgba(255,255,255,0.3)', fontSize:'12px', textAlign:'center' }}>
                ...
                <button
                  ...
                  style={{ background:'none', border:'none', cursor:'pointer', color:'#ff8c00', fontSize:'12px', fontWeight:600, fontFamily:'Sora,sans-serif', padding:0 }}
                >
              ...
              <form onSubmit={handleSignup} className="fade-up" autoComplete="off" style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'8px' }}>
                  {[
                    ...
                    <button key={name} type="button" onClick={() => handleSocialAuth(name)}
                      style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'6px', padding:'11px 8px', borderRadius:'12px', border:'1px solid rgba(255,255,255,0.1)', background:'rgba(255,255,255,0.05)', cursor:'pointer', fontSize:'12px', fontWeight:500, color:'rgba(255,255,255,0.8)', fontFamily:"'Sora',sans-serif", transition:'all 0.15s' }}
