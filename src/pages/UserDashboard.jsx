...
import {
  Plus, Save, Loader2, Sparkles, Lock, CheckCircle,
...
// ─── Limites par plan ─────────────────────────────────────────────────────────
const PLAN_LIMITS = {
  basic:    { maxLinks: 3,  maxProfiles: 1, hasStats: false, label: 'BASIC',    color: '#6366f1', emoji: '⚡' },
  pro:      { maxLinks: 7,  maxProfiles: 2, hasStats: true,  label: 'PRO',      color: '#ff8c00', emoji: '🚀' },
  business: { maxLinks: 10, maxProfiles: 2, hasStats: true,  label: 'BUSINESS', color: '#f7c948', emoji: '💼' },
};
...
function PlanLimitBanner({ limits, currentCount, type }) {
  const remaining = limits - currentCount;
  ...
  return (
    <div style={{
      ...
      {isAtLimit && (
        <a href="/#tarifs" style={{ marginLeft:'auto', fontSize:'11px', color:'#ff8c00', fontWeight:600, textDecoration:'none', whiteSpace:'nowrap' }}>
          Changer d'offre →
        </a>
      )}
    </div>
...
function WaveModal({ onClose }) {
  return (
    ...
    >
      ...
      >
        <div style={{ width:'56px', height:'56px', borderRadius:'16px', background:'linear-gradient(135deg,#0057FF,#0099FF)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px', boxShadow:'0 8px 24px rgba(0,87,255,0.4)' }}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="white"><path d="M3 12c0-1.1.9-2 2-2s2 .9 2 2-.9 2-2 2-2-.9-2-2zm5-3c0-1.1.9-2 2-2s2 .9 2 2-.9 2-2 2-2-.9-2-2zm5 0c0-1.1.9-2 2-2s2 .9 2 2-.9 2-2 2-2-.9-2-2zm2 5c0-1.1.9-2 2-2s2 .9 2 2-.9 2-2 2-2-.9-2-2z"/></svg>
        </div>
        <h3 style={{ color:'white', fontSize:'17px', fontWeight:700, marginBottom:'6px' }}>Activation du compte</h3>
        <p style={{ color:'rgba(255,255,255,0.5)', fontSize:'13px', marginBottom:'20px', lineHeight:1.5 }}>Pour activer votre profil et sauvegarder vos modifications, effectuez le paiement via Wave.</p>
        <div style={{ background:'rgba(0,87,255,0.1)', border:'1px solid rgba(0,87,255,0.25)', borderRadius:'14px', padding:'16px', marginBottom:'16px' }}>
        ...
        <div style={{ background:'rgba(255,255,255,0.06)', borderRadius:'12px', padding:'12px', marginBottom:'8px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <div style={{ background:'rgba(255,255,255,0.05)', borderRadius:'12px', padding:'10px 12px', marginBottom:'20px', textAlign:'left' }}>
          <p style={{ color:'rgba(255,255,255,0.5)', fontSize:'11px', lineHeight:1.6 }}>📸 Après paiement, envoyez une capture d'écran au support WhatsApp. Votre compte sera activé dans les <strong style={{ color:'rgba(255,255,255,0.8)' }}>minutes qui suivent</strong>.</p>
        </div>
        <a href="https://wa.me/2250576031212" target="_blank" rel="noopener noreferrer"
          style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'8px', width:'100%', padding:'12px', background:'#25D366', borderRadius:'12px', color:'white', fontSize:'14px', fontWeight:600, textDecoration:'none', marginBottom:'10px', boxShadow:'0 4px 16px rgba(37,211,102,0.35)' }}>
        ...
        <button onClick={onClose} style={{ width:'100%', padding:'10px', background:'transparent', border:'1px solid rgba(255,255,255,0.12)', borderRadius:'12px', color:'rgba(255,255,255,0.5)', fontSize:'13px', cursor:'pointer' }}>Fermer</button>
      </motion.div>
    </motion.div>
  );
}
...
export default function UserDashboard() {
  ...
  const userPlan = user?.user_metadata?.plan.toLowerCase()?.trim() || 'basic';
  ...
  useEffect(() => {
    ...
    const handler = (e) => { if (bgPanelRef.current && !bgPanelRef.current.contains(e.target)) setShowBgPanel(false); };
  ...
  const handleSaveClick = () => {
    if (!isActivated) { setShowWaveModal(true); return; }
    handleSave();
  };
      ...
      <div className="min-h-screen" style={{ background: localProfile.bg_image_url ? undefined : 'linear-gradient(135deg,' + colors.bg1 + ',' + colors.bg2 + ')' }}>
        ...
        <div className="sticky top-0 z-20 bg-black/20 backdrop-blur-lg border-b border-white/10">
          <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'4px', marginTop:'2px' }}>
                  <span style={{ fontSize:'9px', color: limits.color, fontWeight:700, background:`${limits.color}20`, padding:'1px 6px', borderRadius:'20px', letterSpacing:'0.05em' }}>
            ...
            <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    {localProfile.bg_image_url && (
                      <div style={{ position:'relative', borderRadius:'10px', overflow:'hidden', marginBottom:'10px' }}>
                        ...
                        <button onClick={() => { updateLocal({ bg_image_url: null }); setShowBgPanel(false); }} style={{ position:'absolute', top:'6px', right:'6px', width:'24px', height:'24px', borderRadius:'50%', background:'rgba(0,0,0,0.7)', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}><X size={12} color="white" /></button>
                      </div>
                    )}
                    <label style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'8px', background:'rgba(255,255,255,0.05)', border:'2px dashed rgba(255,255,255,0.15)', borderRadius:'10px', padding:'14px', cursor:'pointer' }}>
              ...
              <Button onClick={handleSaveClick} disabled={updateMutation.isPending} className="rounded-xl gap-2" size="sm"
                style={!isActivated ? { background:'rgba(255,255,255,0.15)', border:'1px solid rgba(255,255,255,0.2)' } : {}}>
                {updateMutation.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : !isActivated ? <Lock className="w-3.5 h-3.5" /> : <Save className="w-3.5 h-3.5" />}
                Sauvegarder
              </Button>
        ...
        {!isActivated && (
          <div style={{ background:'rgba(0,87,255,0.15)', borderBottom:'1px solid rgba(0,87,255,0.3)', padding:'10px 16px', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px' }}>
            ...
            <span style={{ color:'#93c5fd', fontSize:'13px' }}>Compte en attente d'activation — cliquez sur <strong>Sauvegarder</strong> pour les instructions de paiement.</span>
          </div>
        )}
              ...
              <div className="bg-white/20 rounded-2xl border border-white/20 px-4 py-3 space-y-3">
                ...
                {localProfile.is_event && (
                  <div className="space-y-2 pt-2 border-t border-white/10">
                    <div style={{ display:'flex', alignItems:'center', gap:'8px' }}><Lock size={13} color="rgba(255,255,255,0.25)" style={{ flexShrink:0 }} /><LockedField placeholder="Nom de l'événement — activez votre compte" /></div>
                    <div style={{ display:'flex', alignItems:'center', gap:'8px' }}><Lock size={13} color="rgba(255,255,255,0.25)" style={{ flexShrink:0 }} /><LockedField placeholder="Date de l'événement" /></div>
                    <div style={{ display:'flex', alignItems:'center', gap:'8px' }}><Lock size={13} color="rgba(255,255,255,0.25)" style={{ flexShrink:0 }} /><LockedField placeholder="Lieu de l'événement" /></div>
                    <div style={{ display:'flex', alignItems:'center', gap:'8px' }}><Lock size={13} color="rgba(255,255,255,0.25)" style={{ flexShrink:0 }} /><LockedField placeholder="Description / programme" /></div>
                    <div style={{ display:'flex', alignItems:'center', gap:'8px' }}><Lock size={13} color="rgba(255,255,255,0.25)" style={{ flexShrink:0 }} /><LockedField placeholder="Lien de réservation" /></div>
                    {/* Image déverrouillée */}
                    <div style={{ background:'rgba(99,102,241,0.08)', border:'1px solid rgba(99,102,241,0.25)', borderRadius:'12px', padding:'10px 12px' }}>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-white/60 text-xs">Image — <span style={{ color:'#a5b4fc' }}>disponible sans activation</span></span>
                      </div>
            ...
            <div className="space-y-4">
              ...
              {limits.hasStats ? (
                ...
                <div style={{ background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'16px', padding:'16px', textAlign:'center' }}>
                  ...
                  <a href="/#tarifs" style={{ display:'inline-flex', alignItems:'center', gap:'6px', background:'rgba(255,140,0,0.15)', border:'1px solid rgba(255,140,0,0.3)', borderRadius:'10px', padding:'7px 14px', color:'#ff8c00', fontSize:'12px', fontWeight:600, textDecoration:'none' }}>
              ...
              <div style={{ background: isActivated ? 'rgba(34,197,94,0.1)' : 'rgba(0,87,255,0.1)', border:'1px solid ' + (isActivated ? 'rgba(34,197,94,0.3)' : 'rgba(0,87,255,0.3)'), borderRadius:'16px', padding:'14px' }}>
                <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                  ...
                  <div>
                    <p style={{ color:'white', fontSize:'13px', fontWeight:600, margin:0 }}>{isActivated ? '✅ Compte activé' : '⏳ Compte non activé'}</p>
                    <p style={{ color:'rgba(255,255,255,0.5)', fontSize:'11px', margin:'2px 0 0' }}>{isActivated ? 'Toutes les fonctionnalités sont disponibles' : 'Cliquez sur Sauvegarder pour activer'}</p>
                  </div>
                </div>
                {!isActivated && (
                  <a href="https://wa.me/2250576031212" target="_blank" rel="noopener noreferrer"
